...
static unsigned int sched_clock_rate;
...
/*
 * sched_clock_register() is __init — it must not be called from .text.
 * We save the rate here and call it from a late_initcall (__init context)
 * which runs before free_initmem() discards .init.text.
 */
...
static int __init sky1_sched_clock_register(void)
{
	if (sched_clock_reg)
		sched_clock_register(sky1_read_sched_clock, 64, sched_clock_rate);
	return 0;
}
late_initcall(sky1_sched_clock_register);
