...
/*
 * Some CIX ACPI devices have no _CRS resources. The upstream 7.0 kernel
 * only creates platform devices for ACPI nodes that have _CRS. Walk the
 * namespace and create platform devices for any known CIX HIDs that were
 * skipped.
 */
static const struct acpi_device_id cix_no_crs_ids[] = {
	{ "CIXH2032" },	/* USB 2.0 PHY */
	{ },
};

static int cix_create_missing_pdevs(void)
{
	...
	for (id = cix_no_crs_ids; id->id[0]; id++) {
		for_each_acpi_dev_match(adev, id->id, NULL, -1) {
...
late_initcall(cix_create_missing_pdevs);
