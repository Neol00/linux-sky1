...
static int create_dir(const char *name, umode_t mode)
{
	...
	result = vfs_mkdir(rdr_mnt_idmap, d_inode(path.dentry), dentry, mode,
			   NULL);
	if (!IS_ERR(result)) {
		/* mark as kernel-created inode */
		if (d_inode(result))
			d_inode(result)->i_private = (void *)g_rdr_flag;
		end_creating_path(&path, result);
		return 0;
	}
...
/*
 * follow_page() was removed in Linux 7.0. This function is stubbed out
 * until a replacement using get_user_pages or similar is implemented.
 */
unsigned long dst_get_phy_addr(unsigned long addr)
{
	DST_DBG("dst_get_phy_addr: stubbed out (follow_page removed in 7.0)\n");
	return 0;
}
