...
int sched_isolate_count(const cpumask_t *mask, bool include_offline)
{
	...
	if (include_offline) {
		cpumask_andnot(&count_mask, cpu_possible_mask, cpu_online_mask);
...
{
	if (demand_eval(cluster)) // 判断cluster里core的个数是否需要调整
...
static int test_set_val(struct cluster_data *cluster, unsigned int val)
{
	...
	cpumask_andnot(&disable_mask, cpu_possible_mask, cpu_online_mask);
