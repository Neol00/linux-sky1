...
static int cix_dsp_rproc_addr_init(struct cix_dsp_rproc *rproc_priv)
{
	...
	for (i = 0; i < CIX_MEM_REG_NUM; i++) {
		...
		if (page_is_ram(mem_reg[i].sa >> PAGE_SHIFT))
			p_mem[i].cpu_addr = devm_memremap(dev, mem_reg[i].sa,
							  mem_reg[i].len,
							  MEMREMAP_WB);
		else
			p_mem[i].cpu_addr = devm_ioremap_wc(dev, mem_reg[i].sa,
							    mem_reg[i].len);
		if (IS_ERR_OR_NULL(p_mem[i].cpu_addr)) {
