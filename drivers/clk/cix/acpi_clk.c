					     ...
					     unsigned long parent_rate)
{
	...
	if (ACPI_FAILURE(status)) {
		dev_err(dev, "ACPI GClK failed for clk[%u]\n",
			aclk_hw->clk_id);
		ret = -ENODEV;
		goto OUT;
	}
	...
	if (buf_val[0] != SUCCESS) {
		dev_err(dev, "ACPI GClK clk[%u] rec rate err:%d\n",
	...
	dev_dbg(dev, "acpi_clk recalc_rate: clk[%u] = %lu\n",
		aclk_hw->clk_id, clk_rate);
				...
				unsigned long parent_rate)
{
	...
	dev_dbg(dev, "acpi_clk set_rate: clk[%u] rate=%lu\n",
		aclk_hw->clk_id, rate);
	...
	if (ACPI_FAILURE(status)) {
		dev_err(dev, "ACPI SClK evaluation failed for clk[%u]\n",
			aclk_hw->clk_id);
		ret = -ENODEV;
		goto OUT;
	}
	...
	buf_val[0] = *(u32 *)package->buffer.pointer;
	if (buf_val[0] != SUCCESS) {
		dev_err(dev, "ACPI SClK clk[%u] set rate %lu err:%d\n",
					aclk_hw->clk_id, rate, buf_val[0]);
		goto OUT;
	}

	dev_dbg(dev, "acpi_clk set_rate: clk[%u] rate=%lu OK\n",
		aclk_hw->clk_id, rate);
			...
			void *data)
{
	...
	for (i = 0; i < clk_num; i++) {
		...
		dev_dbg(dev, "clk registered: id[%d] con[%s] dev[%s]\n",
				clk_id, con_id, dname);
	}
					...
					void *context, void **ret_p)
{
	...
	if (ret == 0)
		dev_dbg(dev, "acpi_bus_clk_scan: found CLKT on ACPI device\n");
...
static int cix_acpi_clk_probe(struct platform_device *pdev)
{
	...
	dev_dbg(&pdev->dev, "cix_acpi_clk: probing CIXHA010\n");

	status = acpi_walk_namespace(ACPI_TYPE_ANY, ACPI_ROOT_OBJECT,
				ACPI_UINT32_MAX, acpi_bus_clk_scan,
				NULL, &pdev->dev, NULL);

	dev_dbg(&pdev->dev, "cix_acpi_clk: namespace walk done, status=%d\n",
		(int)status);
