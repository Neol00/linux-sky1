					 ...
					 struct list_head *zorder_list)
{
	...
	list_for_each_entry(node, zorder_list, zlist_node) {
		if (new->base.zpos < node->base.zpos) {
			 ...
			 * different planes.
			 */
			pr_debug("PLANE: %s and PLANE: %s are configured same zpos: %d.\n",
					...
					struct drm_crtc_state *crtc_st)
{
	...
	pr_debug("[CRTC:%d:%s] calculating normalized zpos values\n",
	...
	list_for_each_entry(kplane_st, &zorder_list, zlist_node) {
		...
		pr_debug("[PLANE:%d:%s] zpos:%d, normalized zpos: %d\n",
