				       ...
				       const struct drm_mode_fb_cmd2 *mode_cmd)
{
	...
	if (!obj) {
		pr_debug("Failed to lookup GEM object\n");
		return -ENOENT;
	}
	...
	if (fb->offsets[0] % alignment_header) {
		pr_debug("afbc offset alignment check failed.\n");
		goto check_failed;
	}
	...
	if (min_size > obj->size) {
		pr_debug("afbc size check failed, obj_size: 0x%zx. min_size 0x%llx.\n",
			 obj->size, min_size);
		goto check_failed;
	}
				 ...
				 const struct drm_mode_fb_cmd2 *mode_cmd)
{
	...
	for (i = 0; i < info->num_planes; i++) {
		...
		if ((fb->pitches[i] * block_h) % mdev->chip.bus_width) {
			pr_debug("Pitch[%d]: 0x%x doesn't align to 0x%x\n",
		...
		if (obj->size < min_size) {
			pr_debug("The fb->obj[%d] size: 0x%zx lower than the minimum requirement: 0x%llx.\n",
				 i, obj->size, min_size);
			return -EINVAL;
		}
	}
	...
	if (fb->format->num_planes == 3) {
		if (fb->pitches[1] != fb->pitches[2]) {
			pr_debug("The pitch[1] and [2] are not same\n");
			return -EINVAL;
		}
	}
		   ...
		   const struct drm_mode_fb_cmd2 *mode_cmd)
{
	...
	if (!kfb->format_caps) {
		pr_debug("FMT %x is not supported.\n",
	...
	if (ret < 0) {
		pr_debug("failed to initialize fb\n");
				 ...
				 u32 src_y, u32 src_w, u32 src_h)
{
	...
	if ((src_x + src_w > fb->width) || (src_y + src_h > fb->height)) {
		pr_debug("Invalid source coordinate.\n");
		return -EINVAL;
	}

	if ((src_x % info->hsub) || (src_w % info->hsub) ||
	    (src_y % info->vsub) || (src_h % info->vsub)) {
		pr_debug("Wrong subsampling dimension x:%d, y:%d, w:%d, h:%d for format: %x.\n",
	...
	if ((src_x % block_w) || (src_w % block_w) || (src_y % block_h) ||
	    (src_h % block_h)) {
		pr_debug("x:%d, y:%d, w:%d, h:%d should be multiple of block_w/h for format: %x.\n",
			 src_x, src_y, src_w, src_h, info->format);
		return -EINVAL;
	}
				    ...
				    u32 rot)
{
	...
	supported = linlondp_format_mod_supported(&mdev->fmt_tbl, layer_type,
						  fourcc, modifier, rot);
	if (!supported)
		pr_debug("Layer TYPE: %d doesn't support fb FMT: %p4cc with modifier: 0x%llx.\n",
