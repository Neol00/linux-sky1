...
{
	struct linlondp_wb_timer_data *data = container_of(t, struct linlondp_wb_timer_data, timer);
	...
	job = list_first_entry_or_null(&wb_conn->base.job_queue,
				...
				list_entry);
	if (job) {
		pr_debug("Timer fired! has JOB not signal\n");
					...
					struct drm_atomic_state *state)
{
	...
	if (old && old->self_refresh_active) {
		pr_debug("%s self_refresh_active, do nothing.\n", __func__);
	...
	bool disable_crtc)
{
	...
	if (input_flip_done) {
		...
		if (kcrtc->disable_done != NULL) {
			pr_debug("CRTC%d has last flip done %p do nothing",
					 ...
					 struct drm_atomic_state *state)
{
	...
	pr_debug("CRTC%d_DISABLE: active_pipes: 0x%x, affected: 0x%x\n",
			 ...
			 const struct drm_display_mode *m)
{
	...
	if ((m->clock > 600000) && (m->clock < 600000 * 2))
		...
		pr_debug("engine clk can't satisfy the requirement of %s-clk: %d.\n",
			 m->name, m->clock);
	...
	if (min_pxlclk > clk_round_rate(master->pxlclk, min_pxlclk)) {
		pr_debug("pxlclk doesn't support %lu Hz\n", min_pxlclk);
	...
	if (clk_round_rate(mdev->aclk, min_aclk) < min_aclk) {
		pr_debug("engine clk can't satisfy the requirement of %s-clk: %lu.\n",
			 m->name, min_pxlclk);
				     ...
				     struct drm_display_mode *adjusted_mode)
{
	...
	if (rebuilt_crtcs) {
		...
		pr_debug("%s sidy_by_side. rebuild crtcs",
