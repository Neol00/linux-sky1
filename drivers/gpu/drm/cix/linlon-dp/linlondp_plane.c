					 ...
					 struct linlondp_data_flow_cfg *dflow)
{
	...
	if (dflow->blending_zorder < 0) {
		pr_debug("%s zorder:%d < max_slave_zorder: %d.\n",
	...
	if (!has_bits(dflow->rot, caps->supported_rots)) {
		pr_debug("rotation(0x%x) isn't supported by %p4cc with modifier: 0x%llx.\n",
				       ...
				       struct drm_atomic_state *state)
{
	...
	if (IS_ERR(crtc_st) || !crtc_st->enable) {
		pr_debug("Cannot update plane on a disabled CRTC.\n");
		return -EINVAL;
	}
...
static int linlondp_plane_debugfs_init(struct drm_plane *plane)
{
	...
	if (!dir) {
		pr_debug("Cannot create debugfs dir for %s\n", name);
		return 0;
	}
